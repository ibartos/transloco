 ...
 */
export const useValue = <T>(value: T): UnboundValueProvider<T> => ({ useValue: value });
 ...
 */
export const useClass = <T>(providerClass: Type<T>): UnboundClassProvider<T> => ({ useClass: providerClass });
 ...
 */
export const useExisting = <T>(providerToken: ProviderToken<T>): UnboundExistingProvider<T> => ({ useExisting: providerToken });
 ...
 */
export const useFactory = <F extends (...args: any) => any>( // eslint-disable-line @typescript-eslint/no-explicit-any
  factoryFunction: F,
...
export const bindProvider = <T, U extends T>(
