...
export function observeProperty<T, K extends keyof T>(target: T, key: K): Observable<T[K]> {
    ...
    const originalGetAccessor: GetAccessorWithValueStream | undefined = propertyDescriptor && propertyDescriptor.get;
    ...
    const originalSetAccessor = propertyDescriptor && propertyDescriptor.set;
